...
| 30 | **The contract/award-letter upload is multipart-only (no presigned direct path).** Statements got the direct-to-storage path because 5–10 MB annual reports break Vercel's 4.5 MB body cap; contract PDFs are usually small, so the simple route shipped first. A large scanned contract will fail on Vercel until it gets the same presign flow. | Big scanned contracts can't upload on Vercel | Reuse the statement presign flow with docType CONTRACT when it bites |
| 31 | **Legacy columns kept for old rows:** `ContractDetails.expectedPaymentTerms` (superseded by the structured payment fields; still displayed for pre-2026-07-16 cases) and `FinancialStatement.audited` (boolean kept in sync with `statementType` for old readers). | Two sources of truth if a writer forgets the sync | Drop both after the last legacy case is decided or migrated |
| 32 | **KYC answers are declaration-only this phase** (no SIMAH; no document authenticity checks — explicit client decision). The engine scores what is declared and hard-caps on adverse declarations; the SIMAH cross-checks (over-issuance, conduct mismatch) activate when the integration lands. | An applicant can under-declare; only within-Daman exposure is verifiable | SIMAH integration (fields are already collected) |
